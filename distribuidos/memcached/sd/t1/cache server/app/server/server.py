...
class CacheServiceServicer(cache_service_pb2_grpc.CacheServiceServicer):
    def __init__(self, is_master=True, memcache_server='localhost:11214'):
        self.is_master = is_master
        self.nodes = []
        self.ring = uhashring.HashRing()
        self.mc = memcache.Client([memcache_server], debug=0)
    ...
    def Get(self, request, context):
        if self.is_master:
            ...
            value = self.mc.get(request.key)
            print(f"Retrieving key '{request.key}:{value}' from memcache")
            if value:
    ...
    def Put(self, request, context):
        if self.is_master:
            ...
            print(f"Inserting key '{request.key}' in memcache")
            self.mc.set(request.key, request.value)
    ...
    def Remove(self, request, context):
        if self.is_master:
            ...
            self.mc.delete(request.key)
            return Response(success=True, message="Removed successfully")


def serve(is_master=True, port=50053):
...
if __name__ == '__main__':
    ...
    parser.add_argument("port", default=50053, type=int, help="Port number to start the node on")
    parser.add_argument("--master_ip", default="localhost", help="IP address of the master node (required if node_type is 'slave')")
    parser.add_argument("--master_port", type=int, default=50053, help="Port number of the master node (required if node_type is 'slave')")
